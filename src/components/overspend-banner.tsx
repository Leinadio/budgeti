...
export function OverspendBanner({ items, accountId, months, budgetOf }: {
  items: PendingOverspend[];
  accountId: string;
  months: string[]; // mois affichés, pour retrouver l'index de la colonne
  // Budget en vigueur de ce qui déborde, pour pré-remplir le champ « Permanent » du
  // bloc de décision AU MOIS du dépassement. Fourni tout fait : le budget d'une ligne
  // de récurrent ne se lit pas dans la table par groupe, il vit dans sa case du
  // tableau — que le bandeau, lui, n'a pas sous la main.
  budgetOf: (item: PendingOverspend) => number | null;
}) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-md border border-amber-300 bg-amber-50 p-3 text-sm dark:border-amber-700 dark:bg-amber-950">
      ...
      {items.map((it) => (
        <button
          key={`${it.groupId}-${it.lineId ?? 0}-${it.month}`}
          type="button"
          onClick={() =>
            setDetail(
