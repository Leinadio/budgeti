...
test("un refus laisse le rattachement précédent intact", async () => {
  const env = insertGroup(db, "a1", "Courses", "out", 300, null, "2026-01", null);
  const rec = insertGroup(db, "a1", "Abonnements", "out", 0, null, "2026-01", null);
...
test("refuse une ligne qui n'appartient pas au groupe visé", async () => {
  const a = insertGroup(db, "a1", "Abonnements", "out", 0, null, "2026-01", null);
  const b = insertGroup(db, "a1", "Impôts", "out", 0, null, "2026-01", null);
...
test("refuse de rattacher une transaction à un groupe qui ne vit pas son mois", async () => {
  const juillet = insertGroup(db, "a1", "Sucreries", "out", 40, null, "2026-07", "2026-07");
