...
- Modify: `src/enablebanking/sync.ts` (retirer la catégorisation à l'import)
...
- [ ] **Step 6: Retirer la catégorisation de la synchro**

`src/enablebanking/sync.ts` catégorise encore les transactions à l'import (via `categorize`, `listRules`, `ensureCategory`, `uncategorized`, `setTransactionCategory`) — ces API disparaissent avec les catégories. Le retirer :

- Supprimer les imports `import { categorize } from "../lib/categorize";`, `import { listRules } from "../db/repositories/rules";`, `import { ensureCategory } from "../db/repositories/categories";`.
- Dans l'import du repo transactions, ne garder que `upsertTransaction` (retirer `uncategorized`, `setTransactionCategory`).
- Supprimer le bloc de catégorisation après la boucle d'import (les lignes du type `// Categorize everything still uncategorized.` puis la boucle `for (const t of uncategorized(db)) { ... setTransactionCategory(...) }`).
- Conserver le reste de la synchro (upsert comptes/transactions, `category_id: null` dans l'objet transaction reste valide, la colonne existe encore, dormante).

L'auto-classement se fait désormais à la lecture par `resolveOwnership` (mots-clés de groupe), plus besoin de catégoriser à l'import.

- [ ] **Step 7: Vérifier compilation et suite complète**

Run: `npx tsc --noEmit && npm test`
Expected: aucune erreur ; tous les tests passent. (Les fichiers catégories/règles encore présents — `src/db/repositories/categories.ts`, `rules.ts`, `src/lib/categorize.ts`, écran Catégories, composants — ne sont plus référencés par le code applicatif à ce stade et seront supprimés en Task 8.)

- [ ] **Step 8: Commit**

```bash
git add src/components/group-select-field.tsx src/app/transactions/ src/app/previsionnel/page.tsx src/app/page.tsx src/enablebanking/sync.ts
git commit -m "feat: Transactions rattachées aux groupes + appartenance affichée ; prévisionnel/dashboard/synchro adaptés"
```
...
**Files:**
- Delete: `src/app/categories/` (page + éventuel actions), `src/components/category-select-field.tsx`, `src/components/rule-checkbox-field.tsx`, `src/db/repositories/categories.ts`, `src/db/repositories/rules.ts`, `src/lib/categorize.ts` (et son test `tests/lib/categorize.test.ts` s'il existe)
